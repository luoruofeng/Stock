...
public class CodeResultService {
	...
	private List<Stock> getAllStockesByCode(final String codeStr) {
		List<Stock> allStock = AllStockHandler.getAllStock(stockRepository);
		if (allStock == null)
			return null;

		List<Stock> stocks = allStock.stream().filter((s) -> {
			return s.getCode().equals(codeStr);
	...
	private List<Stock> getAllStockesByCodeAndStartDate(String codeStr, Date startDate) {
		return getFilterStockStream(codeStr, getAfterStartDatePredicate(startDate)).sorted(new StockCloseComparable())
				.collect(Collectors.toList());
	}
	...
	private Double getAvgPrice(String codeStr, Date startDate) {
		return getFilterStockCloseStream(codeStr, getAfterStartDatePredicate(startDate)).summaryStatistics()
				.getAverage();
	}
	...
	public List<Stock> getBelowAvg(String codeStr, Date startDate, Double avg) {
		return getFilterStockStream(codeStr, getAfterStartDatePredicate(startDate).and((stock) -> {
	...
	public List<Stock> getBelowAvgMins(String codeStr, Date startDate, Double avg) {
		return getBelowAvg(codeStr, startDate, avg).stream().sorted(new StockCloseComparable())
				.collect(Collectors.toList());
	}
	...
	public List<Stock> getAboveAvgMaxs(String codeStr, Date startDate, Double avg) {
		List<Stock> stocks = getAboveAvg(codeStr, startDate, avg).stream().sorted(new StockCloseComparable())
				.collect(Collectors.toList());
	...
	public void setCodeResult(Code code) {
		for (Day day : Day.values()) {
			...
			code.getCodeResults().put(day.value,
					caculateCodeResultContent(code, createCodeResult(code.getCode(), day.value), day.value));
		}
	}
